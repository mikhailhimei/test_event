...
Локальное desktop-веб-приложение на **Python + Eel** со встроенным запуском **mitmproxy**. Настройки сохраняются в локальном `settings.json`, а история запросов хранится только в памяти текущего запуска.

Интерфейс написан на React и собирается Vite в каталог `web/`. Именно этот собранный каталог отдаёт Eel — исходники из `frontend/src/` браузеру не доступны.
...
После изменения файлов в `frontend/src/` повторно выполните `npm run build`. Сборку можно проверить без запуска desktop-приложения командой `npm run check`.

## Просмотр массивов

Если тело пойманного запроса — массив из нескольких элементов, каждый элемент отображается отдельным свёрнутым блоком **«Элемент массива N»**. Откройте только интересующие элементы: все значения сразу не разворачиваются.

...
- Supports accordion scenarios; each scenario contains its own comparison rules with:
  ...
  - expected value list split by commas; in strict mode a single expected value is applied to every value found by the key path, while two or more comma-separated values are compared positionally and must have the same count as the found values;

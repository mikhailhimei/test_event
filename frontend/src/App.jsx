...
function App() {
  ...
  useEffect(() => {
    async function loadProxyInfo() {
      try {
        setProxyInfo(await eel.ui_proxy_info()());
      } catch (_) {}
    }
    ...
    <>
      ...
      <main>
        ...
        {activeTab === 'scenarios' && (
          <EntityList
            title="Сценарии"
            items={settings.scenarios}
            empty="Сценарии пока не заданы."
            addLabel="Добавить сценарий"
            onAdd={() => setScenarioEditor({ index: null, value: makeScenario(settings.scenarios.length) })}
            onOpen={(value, index) => setScenarioEditor({ index, value: structuredClone(value) })}
            onDeleteAll={() => updateAndSave((next) => ({ ...next, scenarios: [] }))}
          />
        )}
        {activeTab === 'common' && (
          <EntityList
            title="Общие элементы"
            hint="Набор правил, который можно подключить к сценарию."
            items={settings.commonElements}
            empty="Общие элементы пока не заданы."
            addLabel="Добавить общий элемент"
            onAdd={() => setCommonEditor({ index: null, value: makeCommon(settings.commonElements.length) })}
            onOpen={(value, index) => setCommonEditor({ index, value: structuredClone(value) })}
            onDeleteAll={() => updateAndSave((next) => ({ ...next, commonElements: [] }))}
          />
        )}
        {activeTab === 'settings' && (
          <SettingsTab
            ...
            proxyInfo={proxyInfo}
            openCertificateFolder={openCertificateFolder}
          />
        )}
      </main>
      ...
      {scenarioEditor && (
        <ScenarioModal
          ...
          onSave={(scenario) => saveEntity(updateAndSave, 'scenarios', scenarioEditor.index, scenario).then(() => setScenarioEditor(null))}
          onDelete={() => deleteEntity(updateAndSave, 'scenarios', scenarioEditor.index).then(() => setScenarioEditor(null))}
          onDuplicate={() => duplicateScenario(updateAndSave, scenarioEditor.value).then(() => setScenarioEditor(null))}
        />
      )}
      {commonEditor && (
        <CommonModal
          editor={commonEditor}
          onClose={() => setCommonEditor(null)}
          onSave={(element) => saveEntity(updateAndSave, 'commonElements', commonEditor.index, element).then(() => setCommonEditor(null))}
          onDelete={() => deleteEntity(updateAndSave, 'commonElements', commonEditor.index).then(() => setCommonEditor(null))}
        />
      )}
    </>
  );
